...
namespace Duska.Screens
{
    public class GameCardScreen : GameScreen
    {
        ...
        private void ProcessCardsMessage(string message)
        {
            Debug.WriteLine($"[PROCESANDO MENSAJE] {message}");

            // Solo procesar si el mensaje comienza con CARDS/
            if (!message.StartsWith("CARDS/"))
            {
                Debug.WriteLine("[CARDS] Mensaje ignorado: no es un mensaje de cartas");
                return;
            }
            ...
            if (partes.Length < 5)
            {
                Debug.WriteLine($"[CARDS] Formato incorrecto: {message}");
                return;
            }
            ...
            try
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    if (int.TryParse(partes[i + 1], out int cantidad))
                    {
                        Debug.WriteLine($"[CARDS] Tipo {i}: {cantidad} cartas");
                        ...
                        for (int j = 0; j < cantidad; j++)
                        {
                            ...
                            {
                                nuevasCartas.Add(_cartas[i]);
                                Debug.WriteLine($"[CARDS] Añadida carta tipo {i}");
                            }
                            else
                            {
                                Debug.WriteLine($"[ERROR] La carta {i} es null");
                            }
                        }
                    }
                }

                // IMPORTANTE: Actualizar la lista principal directamente (asegúrate de llamar esto solo desde el hilo principal)
                _cartasDisponibles = nuevasCartas;
                Debug.WriteLine($"[CARDS] Lista actualizada. Total: {_cartasDisponibles.Count}");

                Debug.WriteLine($"[CARDS] Proceso completo. Total: {nuevasCartas.Count}");
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[ERROR] Error procesando cartas: {ex.Message}");
            }
        }

        private void EnviarSolicitudCartas()
        {
            try
            {
                if (server != null && server.Connected)
                {
                    byte[] msg = Encoding.ASCII.GetBytes("9/" + usuario + "/");
                    server.Send(msg);
                    Debug.WriteLine("Solicitud de cartas enviada");
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Error al solicitar cartas: {ex.Message}");
            }
        }
    }
}